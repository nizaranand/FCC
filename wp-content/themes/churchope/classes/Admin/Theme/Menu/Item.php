...
//	}
	...
	public function renderPage()
	{
		...
		if($this->getMenuSlug() != SHORTNAME.'_dummy')
		{
			...
			$html .= '<input name="save_options" type="submit" value="'.__('Save','churchope').'" class="th_save"  />'.
				$save_nonce	
				.'</form>
			<form method="post" id="th_reset"  action="">'.
				$reset_nonce
				.'<input name="reset_options" type="submit" value="'.__('Reset','churchope').'" class="th_reset" />
			</form>';
		}
	...
	public function getActionMessage()
	{
		...
		switch($this->getLastAction())
		{
			case self::ACTION_SAVE:
				$message = '<div id="message" class="updated fade" style="width:450px"><p><strong>'.__('Settings have been saved.','churchope').'</strong></p></div>';
				break;
			case self::ACTION_RESET:
				$message = '<div id="message" class="updated fade" style="width:450px"><p><strong>'.__('Settings have been reset.','churchope').'</strong></p></div>';
				break;
			default :
				$message = '';
				break;
