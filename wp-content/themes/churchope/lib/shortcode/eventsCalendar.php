...
function getEventCalendar($layout, $category, $from, $to, $month=null, $year = null)
{
	...
	$calendar .= '<span class="month">'.__($month_name). ' '.$year. '</span>';
	if($show_next_month)
	{
...
/**
 * HTML of calendar
 * @param array $categories events caregories ID to show, ALL - to show all categories
				...
				if($events && is_array($events) && count($events))
				{
					foreach ($events as $event)
					{
						...
						else
						{
							...
							$shown_category = count($categories) == 1 && in_array(Widget_Event::ALL, $categories);
...
//					}
					...
					if($calendar_event && $events_count_for_day >1)
					{
						$calendar .= "<div class='multi_button'><span>".__('More than one event in this day','churchope')."</span>".sprintf($href_to_events_for_the_day, __('view all events', 'churchope'))."</div>";
